...
fn circular_imports_functions() {
    let mut session = TestSession::new();
    session.try_compile_module("A", "pub fn f() {}").unwrap();
    session
        .try_compile_module("B", "pub fn g() { A::f() }")
        .unwrap();
    session
        .try_compile_module("A", "pub fn f() { B::g() }")
        .unwrap_err()
...
fn circular_imports_type_defs() {
    let mut session = TestSession::new();
    session.try_compile_module("A", "pub struct S;").unwrap();
    session
        .try_compile_module("B", "pub struct T(A::S)")
        .unwrap();
    session
        .try_compile_module("A", "pub struct S(B::T)")
        .unwrap_err()
        .as_circular_import_dependency()
        .unwrap();
}
