...
use crate::r#type::TypeOfNativeValue;
...
pub(crate) fn s(lexeme: LexemeResult, lexer: DefaultLexer) -> Ustr {
    let span = lexeme.expect("lexer has an error").span();
...
where
    T: Any + Clone + fmt::Debug + std::cmp::Eq + TypeOfNativeValue + 'static,
{
    Expr::new(Literal(Value::native(value)), span)
}

/// Parse an integer, if it is too big, return an error
pub(crate) fn parse_num<F>(s: &str, span: Span) -> Expr
where
    F: FromStr + Bounded + Display + Clone + Debug + Eq + TypeOfNativeValue + 'static,
{
    match s.parse::<F>() {
...
pub(crate) fn make_proj_or_float(
    ...
    if let Literal(literal) = &lhs.kind {
    ...
    Expr::new(Project(B::new(lhs), index), span)
}
...
pub(crate) fn make_tuple(args: Vec<Expr>, span: Span) -> Expr {
    ...
    for arg in &args {
        if let Literal(val) = &arg.kind {
            values.push(val.clone());
        } else {
            return Expr::new(Tuple(args), span);
        }
    }
    Expr::new(Literal(Value::tuple(values)), span)
}
...
pub(crate) fn make_if_without_else(cond: Expr, if_true: Expr, span: Span) -> Expr {
    ...
    let unit_val = Expr::new(Literal(Value::native(())), if_true_span);
