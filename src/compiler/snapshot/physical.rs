...
impl CompiledPhysicalMirSnapshot {
    pub(crate) fn capture(
        ...
        let name = |native: &crate::types::r#type::BareNativeTypeB| catalog.canonical_name(native);
    ...
    pub(crate) fn restore(
        ...
        for (index, body) in self.functions.iter().enumerate() {
            ...
            let requires_body = index >= optimized.entry_count()
                ...
                || module.get_function_by_id(id).is_some_and(|function| {
                    matches!(
                        function.origin,
                        crate::module::function::CallableOrigin::StructuralFieldAddressor { .. }
                    )
                });
        ...
        let artifacts = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
