...
impl<'a> Interpreter<'a> {
    ...
    fn exec(
        ...
        Ok(match instr.view() {
            ...
            InstructionView::Subfield { .. } => {
                self.exec_subfield(slots, &instr.operands, def.unwrap());
                Step::Advance
            }
            ...
            InstructionView::Store => {
                self.exec_store(slots, &instr.operands)?;
                Step::Advance
            }
            ...
            InstructionView::CompareEqual => {
                self.exec_compare_equal(slots, &instr.operands, def.unwrap());
                Step::Advance
            }
    ...
    fn exec_subfield(
        ...
        let index = self.value_operand(slots, &operands[1]);
        let index = *index
    ...
    /// dictionary operand to its interned id, reads entry `entry_index` (a method function value
    /// or an associated const) straight from the impl arena, and materializes it into a fresh cell
    ...
    fn exec_store(
        ...
        let v = match &operands[0] {
            ...
            op => self.value_operand(slots, op),
        };
        let place = self.place_operand(slots, &operands[1]);
        self.store(v, &place)
    }

    /// Executes a `memcpy` instruction: a pure copy. The source pointee owns no resource
    /// (resource-owning copies are lowered through `Value::clone`), so `load`+`store` duplicates
    /// it — and for a scalar pointee `read_copy` leaves the source live. The debug assert enforces
    /// that contract: an owned pointee here would be a move mislabeled as a copy (which on a real
    /// backend is a shallow copy without drop-cancellation — a double-free / leak).
    #[inline(never)]
    fn exec_memcpy(
        &mut self,
        slots: &mut FxHashMap<ssa::Value, Binding>,
    ...
    /// Executes a `comp_eq` instruction: a lowered `match`'s comparison. Both operands are read
    /// *non-consumingly*: a scrutinee place is borrowed (never moved), so it stays live for the
    /// remaining alternatives and for the arm body, and the literal pattern is materialized. The
    /// comparison is `LiteralValue` equality over the snapshots — exactly the HIR interpreter's
    /// `eval_case` (`scrutinee.to_literal_value() == pattern`).
    #[inline(never)]
    fn exec_compare_equal(
        ...
        let a = self.operand_literal(slots, &operands[0]);
        let b = self.operand_literal(slots, &operands[1]);
        slots.insert(def, Binding::Value(Value::native(a == b)));
    }
