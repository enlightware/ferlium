...
impl<'a> Emitter<'a> {
    ...
    fn value_drop_sibling_of_clone(&self, clone: FunctionId) -> FunctionReference {
        let env = ModuleEnv::new(self.module, self.others);
        let value_trait_id = env.expect_std_trait_id(VALUE_TRAIT_NAME);
        let trait_def = env.trait_def(value_trait_id);
    ...
    fn lower_as_place(&mut self, node: &ENode) -> ssa::Value {
        ...
        match &node.kind {
            ...
            K::CallDictionaryMethod(n) => {
                ...
                let (method, arguments) = self.lower_dictionary_method_target(node, n);
                self.emit_call_as_place(node, method, arguments, &n.ty)
            }
    ...
    fn lower_value_into(&mut self, node: &ENode, destination: Option<ssa::Value>) {
        ...
        match &node.kind {
            ...
            K::CallDictionaryMethod(n) => {
                ...
                let (method, arguments) = self.lower_dictionary_method_target(node, n);
                self.emit_call_into(node, method, arguments, &n.ty, destination);
            }
