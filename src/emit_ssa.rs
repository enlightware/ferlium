...
impl<'a> Emitter<'a> {
    ...
    /// transfers such a value into the callee's frame, but in the by-pointer SSA ABI every
    ...
    /// Pushes the dedicated scope a `WithYielded` opens: its trailing cleanup action is the
    /// `end_project` that runs the accessor slide for the projection `place`, preceded (in exit
    /// order: `end_project` first, then the drops) by the drop obligations of `deferred_cleanup` —
    /// the cleanup locals of any block accessor, which run after the slide (mirroring
    /// `eval_block_accessor_until_yield`, which pushes the block cleanup onto the epilogue).
    /// Lowering the body inside this scope makes the slide run after the body's own drops, on
    /// every exit (normal, transfer, error pad).
    fn enter_projection_scope(&mut self, place: ssa::Value, deferred_cleanup: &[LocalDeclId]) {
        let mut actions = self.drop_actions(deferred_cleanup);
        actions.push(CleanupAction::EndProject { place });
        self.context.scopes.push(Scope { actions, pad: None });
    }
    ...
    fn int_constant_place(&mut self, span: Location, value: isize) -> ssa::Value {
        ...
        self.insert(Instruction::store(span, value, place.clone()));
        place
    }

    /// Interns a typed Ferlium `int` in the function-local constant pool.
    fn int_constant(&mut self, value: isize) -> ssa::Value {
    ...
    fn store_int_field(
        ...
        let place = self
            .insert(Instruction::subfield(span, dest.clone(), index, ty))
            .unwrap();
        let value = self.int_constant(value);
        self.insert(Instruction::store(span, value, place));
    }
    ...
    fn lower_with_yielded_enter(&mut self, n: &hir::WithYielded<Elaborated>) {
        ...
        if self.current_block_is_terminated() {
            ...
            self.enter_projection_scope(dummy, &[]);
            return;
        }

        let (callee, extra_arguments, visible_arguments) = match &accessor.kind {
    ...
    fn lower_case_pattern(&mut self, pattern: &LiteralValue) -> ssa::Value {
        ssa::Value::Pattern(containers::b(pattern.clone()))
    }
    ...
    /// Lowers a HIR subscript-evidence node to a symbolic SSA subscript operand.
    ///
    ...
    fn lower_subscript_operand(&mut self, node: &ENode) -> ssa::Value {
        use hir::NodeKind as K;
        match &node.kind {
            K::GetSubscript(n) => ssa::Value::Subscript(n.subscript),
    ...
    fn lower_subscript_member(
        ...
        self.insert(Instruction::subscript_member(
            node.span,
            subscript,
            n.mut_member,
            member_ty,
    ...
    fn lower_value_into(&mut self, node: &ENode, destination: Option<ssa::Value>) {
        ...
        match &node.kind {
            ...
            K::Assign(n) => {
                ...
                // before a later field reads it (for example `a = (a.1, a.0)`). A later SSA
            ...
            K::GetSubscript(n) => {
                ...
                let subscript = ssa::Value::Subscript(n.subscript);
            ...
            K::BuildSubscriptValue(n) => {
                ...
                let evidence: Vec<ssa::Value> = n
                    .evidence_captures
                    .iter()
                    .map(|e| self.lower_extra_argument(&self.hir_arena[*e]))
                    .collect();
                let value = self
                    .insert(Instruction::build_subscript(
