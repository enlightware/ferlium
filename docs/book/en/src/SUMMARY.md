# Welcome to Ferlium

- [Introduction](./introduction.md)
- [Getting Started](./getting-started.md)

# Core Language

- [Values and Types](./values-and-types.md)
- [Bindings, Scope and Mutability](./scope-and-mutability.md)
- [Functions and Type Inference](./functions-and-type-inference.md)
- [Control Flow and Pattern Matching](./control-flow.md)

# Structured Data

- [Arrays, Ranges, and Iteration](./array-ranges-and-iteration.md)
- [User-Defined Types](./user-defined-types.md)
- [Pattern Matching](./pattern-matching.md)

# Types and Abstraction

- [Functions as Values](./functions-as-values.md)
- [Type Abstraction](./type-abstraction.md)
- [Explicit Types](./explicit-types.md)
- [Trait Implementations and Coherence](./trait-implementations-and-coherence.md)

# Standard Library

- [Sequence Processing](./sequence-processing.md)
- [Unordered Collections](./unordered-collections.md)
- [Numbers](./numbers.md)
- [Bitwise Operations](./bitwise-operations.md)

# Effects and Execution

- [Effects](./effects.md)
- [Evaluation and Mutable State](./evaluation-and-mutable-state.md)
